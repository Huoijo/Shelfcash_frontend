...
export async function GET(request?: Request) {
  ...
  const zipPath = path.join(process.cwd(), "public", "shelfcash_sample_dataset.zip");
  if (fs.existsSync(zipPath)) {
    const fileBuffer = fs.readFileSync(zipPath);
    return new Response(fileBuffer, {
      headers: {
        "content-type": "application/zip",
        "content-disposition": 'attachment; filename="shelfcash_sample_dataset.zip"',
      },
    });
  }

  const rootZipPath = path.join(process.cwd(), "shelfcash_sample_dataset.zip");
  if (fs.existsSync(rootZipPath)) {
    const fileBuffer = fs.readFileSync(rootZipPath);
    return new Response(fileBuffer, {
      headers: {
        "content-type": "application/zip",
        "content-disposition": 'attachment; filename="shelfcash_sample_dataset.zip"',
