...
function DemandExplanationDialog({ row, onClose }: { row: DecisionDemandView; onClose: () => void }) {
  ...
  return <div className="procurement-drawer-layer">
    ...
    <aside aria-labelledby="procurement-drawer-title" aria-modal="true" className="procurement-drawer" ref={drawer} role="dialog">
      ...
      </> : <Notice tone="info">Hệ thống chưa trả về dữ liệu đóng góp theo món cho nguyên liệu này.</Notice>}
    </aside>
  </div>;
}
...
export function ProcurementDecisionWorkspace({ data, plan, decision, busy, onRunAgain }: { data: BootstrapData; plan: PlanResponse; decision: DecisionPackage | null; busy: boolean; onRunAgain: () => void }) {
  ...
  return <>
    <header className="procurement-workspace-header"><div><span className="eyebrow">Cập nhật từ Decision Run gần nhất</span><h1>Kế hoạch nhập hàng</h1><p>Dựa trên dự báo từ {dateWindowLabel(asOfDate || undefined, horizon)}</p></div><Button busy={busy} onClick={onRunAgain} variant="secondary">Chạy lại kế hoạch</Button></header>
    ...
    <section className="procurement-candidates" aria-labelledby="procurement-candidates-title"><div className="procurement-section-heading"><div><span className="eyebrow">Kết quả từ optimizer</span><h2 id="procurement-candidates-title">Phương án mô phỏng chưa đạt điều kiện</h2></div></div>{view.strategies.length ? <div className="procurement-strategy-list">{view.strategies.map((strategy) => <CandidateStrategy key={strategy.key} strategy={strategy} />)}</div> : <Notice tone="info">Chưa có phương án mô phỏng để đối chiếu.</Notice>}</section>
...
export function ProcurementLoadingWorkspace({ onRunAgain }: { onRunAgain: () => void }) {
  return <><header className="procurement-workspace-header"><div><span className="eyebrow">Đang cập nhật kết quả</span><h1>Kế hoạch nhập hàng</h1><p>Hệ thống đang dự báo sản phẩm, tính nhu cầu nguyên liệu và kiểm tra phương án nhập.</p></div><Button busy disabled onClick={onRunAgain} variant="secondary">Đang chạy kế hoạch</Button></header><ol className="procurement-run-status" aria-label="Tiến trình kế hoạch nhập hàng"><li><strong>Dự báo bán hàng</strong><span>Đang xử lý</span></li><li><strong>Nhu cầu nguyên liệu</strong><span>Đang chờ kết quả</span></li><li><strong>Kế hoạch nhập</strong><span>Đang kiểm tra khả thi</span></li></ol><section aria-label="Đang tải nhu cầu nguyên liệu" className="procurement-loading"><span /><span /><span /></section></>;
}
